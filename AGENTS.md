...
### Key Rules & Constraints (Strict 100% Free Quota Policy - 0 บาท):
1. **Strict 100% Free Tier (0 THB Budget)**:
   - **LINE OA**: ใช้งานผ่าน **Rich Menu 6 ช่อง (Reply Message Token ฟรี 100% ไม่จำกัดจำนวนครั้ง)** ห้ามยิง Push Message รายวันเพื่อหลีกเลี่ยงค่าบริการแพ็กเกจ
   - **Telegram Bot**: ใช้สำหรับ **Push Broadcast อัตโนมัติ 2 รอบ/วัน (08:00 และ 18:00 น.) ฟรีตลอดชีพ 100%**
2. **Centralized 1-Call Summarization**: Generate central digest summaries **once per category per round** (08:00 & 18:00 TH), store in R2/Supabase, and dispatch copies to matching subscribers. Never call AI per individual subscriber.
3. **Deterministic / Rule-Based Signals**: Stock/Gold signals must be calculated using clear mathematical rules (e.g. price % change, volume spikes, breakout levels), never hallucinated by LLMs.
