export interface AuthUser { id: string; name: string; email: string; provider: 'password' | 'google'; role: 'developer' | 'member'; plan: 'free' | 'pro' | 'team'; subscriptionStatus: 'inactive' | 'active' | 'past_due'; createdAt: string; }
...
export const authService = {
  ...
  forgotPassword: (email: string) => request('/api/auth/forgot-password', { method: 'POST', body: JSON.stringify({ email }) }) as Promise<{ message: string; resetUrl?: string }>,
  resetPassword: (token: string, password: string) => request('/api/auth/reset-password', { method: 'POST', body: JSON.stringify({ token, password }) }) as Promise<{ user: AuthUser }>,
