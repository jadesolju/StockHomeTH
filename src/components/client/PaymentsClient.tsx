...
import { Loader2, Zap, Crown, AlertCircle, ShieldCheck, ArrowRight, Check, Sparkles, CreditCard, QrCode, History, CheckCircle2 } from 'lucide-react';
...
export default function PaymentsClient() {
  ...
  // Auto-switch tab if URL contains ?tab=topup or ?tab=subscription
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const tabParam = params.get('tab');
      if (tabParam === 'topup' || window.location.hash === '#topup') {
        setActiveTab('topup');
      ...
      <div className="payment-page-container">
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            ...
            >
              <span>รับข่าวสารฟรีผ่าน Telegram Bot ↗</span>
            </a>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '28px', gap: '10px' }}>
          ...
          <div className="flex ios-segmented-control" style={{ padding: '4px', maxWidth: '420px', width: '100%' }}>
            <button
            ...
            >
              <Crown size={16} />
              <span>แผนสมาชิก (Subscription Plans)</span>
            </button>
          </div>
        </div>
