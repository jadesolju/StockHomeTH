...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      console.warn(`[Chart API] Python chart fetch failed for ${symbol}, trying Yahoo HTTP chart API:`, pyErr);
    }

    // 2. Fallback: Direct Yahoo Finance Chart HTTP query
    ...
    return NextResponse.json({ error: 'No chart data found for ' + symbol }, { status: 404 });
  } catch (error: any) {
