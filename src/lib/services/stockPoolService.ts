    ...
    if (supabase) {
      ...
      if (!error && data && data.price > 0) {
        const item: StockPoolItem = {
          ...
          peRatio: data.pe_ratio ? Number(data.pe_ratio) : undefined,
          dividendYield: data.dividend_yield ? Number(data.dividend_yield) : undefined,
          high52w: data.high_52w ? Number(data.high_52w) : undefined,
          low52w: data.low_52w ? Number(data.low_52w) : undefined,
          volume: data.volume || '—',
          aiInsight: data.ai_insight || undefined,
          description: data.description || undefined,
          sparkline7d: Array.isArray(data.sparkline_7d) ? data.sparkline_7d : [],
          analystRating: (data.analyst_rating as any) || 'Hold',
          targetPrice: data.target_price ? Number(data.target_price) : undefined,
...
export async function fetchPendingAnalysisBatch(limit = 10): Promise<StockPoolItem[]> {
  ...
  try {
    ...
    return data.map((d: any) => ({
      ...
      peRatio: d.pe_ratio ? Number(d.pe_ratio) : undefined,
      dividendYield: d.dividend_yield ? Number(d.dividend_yield) : undefined,
      high52w: d.high_52w ? Number(d.high_52w) : undefined,
      low52w: d.low_52w ? Number(d.low_52w) : undefined,
      volume: d.volume,
