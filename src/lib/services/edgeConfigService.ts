 ...
 * 
 * Provides ultra-low-latency flag retrieval (<1ms at Edge CDN) and dynamic circuit breaker tripping
 * to instantly protect Supabase/R2 backend and enable maintenance/static fallback modes.
 */
...
export async function getEdgeConfigState(): Promise<EdgeConfigState> {
  ...
  try {
    ...
    return {
      ...
      disabled_features: Array.isArray(items.disabled_features) ? (items.disabled_features as string[]) : [],
      egress_budget_guard: Boolean(items.egress_budget_guard ?? false),
      ...items,
    };
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    console.error('[EdgeConfigService] Error fetching Edge Config state:', message);
    return DEFAULT_STATE;
  }
}

/**
 * Checks if a specific feature is currently disabled via Edge Config
 */
export async function isFeatureDisabled(featureName: string): Promise<boolean> {
  const state = await getEdgeConfigState();
  return state.disabled_features.includes(featureName);
}
...
export async function tripCircuitBreaker(
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[EdgeConfigService] Error tripping circuit breaker '${flagKey}':`, message);
    return false;
  }
}
